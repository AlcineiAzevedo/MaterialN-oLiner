...
#Exemplo com dados de experimento em dbc
DadosDBC=read.table("DadosDBC.txt",h=T)
?fit_regression

modelo=y~a*x^b/(c^b+x^b)
chute=list(a=36,b=2,c=30)
AjusteDBC=fit_regression(Data =DadosDBC,model = modelo,start = chute,design=3)
plot_regression(AjusteDBC,plot=7,xlabel = "Doses",ylabel = "Altura (cm)")
